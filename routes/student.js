...
router.post("/apply", varifyStudent, async (req, res) => {
    ...
    try {
        ...
        if (appliedjob) {
            return res.status(200).json({ message: "Already applied for job" });
        }
        ...
        if (!job) {
            return res.status(200).json({ error: "No such a job found" });
        }
        console.log(job);
        ...
        res.json({ result, message: "applied for job" });
    } catch (err) {
        res.status(400).json({
            error: "error! can't apply for job, try again leter",
        });
    }
});
